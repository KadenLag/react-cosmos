...
var buildPathMatchers = function buildPathMatchers(componentName, fixturesDir) {
  return ['/' + fixturesDir + '/' + componentName + '/([^/]+)\\.(' + _fixtureExtensions.FIXTURE_EXTENSIONS_REGEX + ')$', '/' + componentName + '/' + fixturesDir + '/([^/]+)\\.(' + _fixtureExtensions.FIXTURE_EXTENSIONS_REGEX + ')$', '/' + componentName + '/([^/]+)\\.(' + _fixtureExtensions.FIXTURE_EXTENSIONS_REGEX + ')$'];
};

var buildDeprecatedPathMatchers = function buildDeprecatedPathMatchers(componentName, fixturesDir) {
  return ['/' + componentName + '/src/' + fixturesDir + '/([^/]+)\\.(' + _fixtureExtensions.FIXTURE_EXTENSIONS_REGEX + ')$'];
};
...
var matchFixturePath = function matchFixturePath(fixturePath, componentName, fixturesDir) {
  ...
  for (var _i = 0; _i < deprecatedMatchers.length; _i += 1) {
    ...
    if (_matchResult) {
      console.warn('The fixture path "' + fixturePath + '" for component name "' + componentName + '" has matched a deprecated pattern ' + deprecatedMatchers[_i] + ', use getFixturePathsForComponent in your cosmos.config.js to handle special cases instead');
