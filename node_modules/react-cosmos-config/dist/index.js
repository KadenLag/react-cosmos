...
function getCosmosConfig(cosmosConfigPath) {
  ...
  if (!configExist(configPath)) {
    if (_yargs.argv.config) {
      ...
      (0, _log.warn)('[Cosmos] No config file found at ' + _relPath + ', using defaults');
    } else {
...
function generateCosmosConfig() {
  ...
  if ((0, _server.moduleExists)((0, _server.resolveUserPath)(rootPath, craWebpackConfigPath))) {
    _fs2.default.writeFileSync(configPath, _configTemplates.CRA_COSMOS_CONFIG, 'utf8');
...
function getUserConfigPath(customConfigPath) {
  var loosePath = _path2.default.resolve(process.cwd(), customConfigPath || _yargs.argv.config || 'cosmos.config.js');
